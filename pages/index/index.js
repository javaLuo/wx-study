...
Page({
  data: {
    ...
    scrollTops:[0,0,0,0,0,0,0,0], // 每一项的scrollTop,这么做是因为放在datas中，更新datas时会重复触发scroll-view的设置top事件，所以单独拿出来
    showa:[[],[],[],[],[],[],[],[]],
  },
  ...
  _updateScrollTop: function(index){
    ...
    if (this.data.scrollTops[index] !== 0){ // 不等于0说明已经设置过了
      return;
    }
  ...
  onScrollTap(e){
    console.log("点击触发：", e);
    if (this.data.aOpen){ // 已经是答案全部高亮状态，直接返回
      return;
    }
